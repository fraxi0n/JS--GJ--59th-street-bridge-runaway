...
//---------------------------------------
      ...
      //////////-NB de voies 
       ...
       if     ( R1 == 2 )  { R2 = RandomINT(1,3) }
      else if ( R1 == 3 )  { R2=RandomINT(1,2) }
      else                 { R2=1   }
...
function FindNumSegment (pNumObstacle) { 
  ...
  else {x= Math.floor ( pNumObstacle/nbObstacleSegment)+1 }
...
if ( pNumObstacle%nbObstacleSegment == 0 )
{ x = nbObstacleSegment}
else { x =  pNumObstacle%nbObstacleSegment }

return x
}

function CalObsPosX (pNumObstacle ) { return  distDepart+distSegment * (FindNumSegment(pNumObstacle)-1)+distObstacle*FindNumObstacle(pNumObstacle)- distVoiture   }
...
function LaunchGame() {
  ...
  for(let S= 1 ; S<= nbSegment;S++ ){ nextMessage [S] = distDepart + distSegment*(S-1) +distSegment*apparitionMessage
...
function load (){
  ...
  for (let S = 1; S<=50; S++) { //console.log ( FindNumSegment(S),FindNumObstacle(S), segment [FindNumSegment(S)].voieDepart, segment[FindNumSegment(S)].nbVoie  ) 
...
function KeyDown (t)   //      ELSE IF A TESTER 
{
  ...
  if ( GameMod == "MENU" ){ 
   ...
   if ( t.code == "ArrowUp" && voiture.V> 1 ){ voiture.V = voiture.V-1
   voiture.y =V[voiture.V] }
  
  
   if ( t.code == "ArrowDown" && voiture.V< 4 ){ voiture.V = voiture.V+1
   voiture.y =V[voiture.V] }
...
function update (){
  ...
  if ( roadX + roadEcartX<= 0 ){ roadX= roadX+ roadEcartX }
 ...
 //---------------------
  ...
  if (  FindNumObstacle( nextObstacle) !=1 ) { 
    if ( segment[  FindNumSegment( nextObstacle) ] .nbVoie == 2  ){
      ...
      if ( voiture.V == 1 || voiture.V == 2 ){  BOOM() }
      } else if ( segment[  FindNumSegment( nextObstacle)  
      ] .voieDepart == 2 ){
      if ( voiture.V == 1 || voiture.V == 4 ){  BOOM() }
      } else { if ( voiture.V == 3 || voiture.V == 4 ){  BOOM() } } 
...
if ( GameMod == "MENU" ){  pCtx.fillText("Press ENTER to start", 300,50) }
